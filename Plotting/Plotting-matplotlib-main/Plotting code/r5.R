getwd()
setwd('/Users/oneryigit/desktop/r5')
library(foreign)
library(caTools)
library(dplyr)
library(ggplot2)
...
df=read.dta('grade_clean.dta')

data=dummy_cols(df, remove_first_dummy = TRUE,
           select_columns = c('major', 'gender' ,'schoollevel'))

clean_df=data[,c('finalgrade', 'discfinal',
        'major_Political Science',
        'gender_M',"schoollevel_Junior" ,
        "schoollevel_Sophomore","schoollevel_Senior")]

colnames(clean_df)[3]="political"

lm=lm(finalgrade ~ discfinal + political + gender_M + schoollevel_Junior+
     schoollevel_Sophomore + schoollevel_Senior, data=clean_df)

stargazer(lm, title="Regression Results")
